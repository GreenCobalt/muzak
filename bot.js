...
async function createAudio(url) {
	try {
		...
		if (ytdl.validateURL(url)) {
			...
			let mediumFormats = audioFormats.filter(
				format => format.audioQuality === 'AUDIO_QUALITY_MEDIUM'
			);
			if (mediumFormats.length == 0) quality = audioFormats[0].itag;
			else quality = mediumFormats[mediumFormats.length - 1].itag;

			res = await createAudioResource(
				await ytdl(url, { quality: quality, highWaterMark: 1 << 25 })
			);
		}
	} catch (e) {
		console.error(e);
...
async function queueCmd(interaction, guildId) {
	...
	for (s in serverQueue) {
		...
		if (s == 0) {
			queueMsg += ` (Now Playing - \`${msToMS(
				player._state.resource.playbackDuration
			)}\` / \`${msToMS(serverQueue[s].length)}\`)`;
		} else {
...
async function playCmd(interaction, guildId) {
	...
	if (!permissions.has('CONNECT') || !permissions.has('SPEAK')) {
		const embed = new EmbedBuilder().setTitle(
			"🔇 I need the permissions to join and speak in the voice channel you're in!"
		);
		const messageId = await interaction.editReply({ embeds: [embed] });
		return;
	}
	...
	try {
		if (ytdl.validateURL(url)) {
			...
			embed = new EmbedBuilder().addFields([
				{
					name: `Importing songs from playlist`,
					value: `This might take a while...`
				}
			]);
			...
			try {
				...
				embed = new EmbedBuilder().addFields([
					{
						name: `Could not import playlist.`,
						value: `**Is the playlist private?**`
					}
				]);
				interaction.editReply({ embeds: [embed] });
			}
			...
			spotifyToYoutube(sArr).then(data => {
				for (var d in data) {
					ytdl
						.getInfo(`https://www.youtube.com/watch?v=${data[d]}`)
						.then(songInfo => {
							newSong = {
								title: songInfo.videoDetails.title,
								url: songInfo.videoDetails.video_url,
				...
				const interval = setInterval(function () {
					if (sArr.length != songArr.length) {
						...
						embed = new EmbedBuilder().addFields([
							{ name: `🎶  Added playlist to queue!`, value: `Done` }
						]);
			...
			await interaction.editReply({
				content: `Searching YouTube for ${url}...`
			});
